...
kdk_config_create(kdk_mem_pool_t *mem_pool, kdk_uint32 mem_pool_size, kdk_char32 *config_file)
{
    ...
    config->file_handle = fopen(config_file, "r");
    if(config->file_handle == KDK_NULL)
    {
    ...
    config->coll = kdk_hash_table_create(config->mem_pool, mem_pool_size, 103);
    if(config->coll == KDK_NULL)
    {
        kdk_config_destroy(config);
        config = KDK_NULL;
        return KDK_NULL;
...
kdk_config_init(kdk_config_t *config)
{
    ...
    while(fgets(line, CONFIG_LINE_LEN, config->file_handle) != KDK_NULL) 
    {
...
kdk_config_set_value(kdk_config_t *config, kdk_char32 *section, kdk_char32 *key, kdk_char32 *value)
{
    ...
    return kdk_hash_table_set_value(config->coll, (kdk_char32 *)hash_key, (void *)value, strlen(value));
}
...
kdk_config_get_value(kdk_config_t *config, kdk_char32 *section, kdk_char32 *key, kdk_char32 *value)
{
    ...
    ret_value = (kdk_char32 *)kdk_hash_table_get_value(config->coll, hash_key);
...
kdk_config_destroy(kdk_config_t *config)
{
    ...
    if(config->file_handle != KDK_NULL)
    {
        ...
        config->file_handle = KDK_NULL;
    }

    if(config->coll != KDK_NULL)
    {
        kdk_hash_table_destroy(config->coll);
        config->coll = KDK_NULL;
    }

    if(config->mem_pool != KDK_NULL && config->mem_pool_type == OWN_MEM_POOL)
    {
        kdk_mem_pool_destroy(config->mem_pool);
        config->mem_pool = KDK_NULL;
    }
